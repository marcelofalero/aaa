...
def main():
    ...
    if pr_url:
        ...
        # Use --auto --merge to merge as soon as checks pass, or just --merge to do it immediately
        merge_result = run_command(["gh", "pr", "merge", "--merge", "--delete-branch"])
        if merge_result:
            print("Successfully merged and deleted branch.")
        else:
            print("Merge failed or is waiting for checks. You can check status with 'gh pr status'")
    else:
