...
with open('site/content/warships/ship-construction/01-class-hull.md', 'r') as f:
    original = f.read()

# Extract tables
tables_match = re.search(r'(### Table 5-1a: Military Hulls.*?)(Hull Points: The number of hull points available)', original, re.DOTALL)
if tables_match:
    tables = tables_match.group(1).strip()
    tables_desc = tables_match.group(2) + original[tables_match.end(2):original.find('Medium Freighter')].strip()
else:
