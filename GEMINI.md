...
Please strictly follow the guidelines established in the `site/AGENTS.md` file for this project. 
...
8. **Explicit Readiness:** Do not merge or push changes to the `main` branch unless the user explicitly confirms that the work is ready. Since the "Required Reviews" protection has been removed, you may merge pull requests yourself once the user gives the the explicit order to publish it.
9. **Internal Linking:** Use the `{{< relref "path/to/page" >}}` shortcode for all internal links within Markdown content. This ensures build-time path verification and automatic multilingual support (correctly linking to the English or Spanish version of a page based on context).
10. In YAML files long text should use |- format and have no more than 80 characters per line.
