+++
title = "Enseñar"
description = "Capacidad para instruir a otros en el uso de una habilidad perteneciente al grupo Abierto."
weight = 8
attribute = "WIL"
category = "Social"
+++

This broad skill represents a hero's ability to instruct others in Open group skills.

### Results
Instruction from a teacher can decrease the cost for a character when buying or improving a skill. A teacher can only teach skills he possesses, and can only teach a specialty up to one-half his current rank.
- **Critical Failure:** student doesn't learn or improve at this time.
- **Failure:** student must pay full skill cost.
- **Ordinary:** student pays cost -1.
- **Good:** student pays cost -2.
- **Amazing:** student pays cost -3.

## Specialty Skills

{{% list %}}
- **Teach, Specific Field:** Ability to teach skills associated with a profession.

### Enseñar [campo específico]
*This skill can't be used untrained.*

For each separate purchase of this specialty, the hero selects a profession (Combat Spec, Diplomat, Free Agent, or Tech Op) and becomes able to teach skills associated with that profession in addition to those belonging to the Open group.
| Results | Description |
| :--- | :--- |
| **Ordinary** | Success indicates that the student pays cost -1. |
| **Good** | Success indicates that the student pays cost -2. |
| **Amazing** | Success indicates that the student pays cost -3. |
