+++
title = "Operación técnica"
description = "Operarios técnicos e ingenieros."
weight = 4
+++

# Operación técnica (Tech Op)

Los Operarios Técnicos (Tech Ops) son los maestros del universo de alta tecnología. Ya sean pilotos, médicos o ingenieros, dependen de su genialidad, agilidad y formación especializada para operar y mantener la compleja maquinaria del futuro.

## Requisitos
- **DEX 9+, INT 9+**

## Beneficios de Profesión
- **Incremento en la Tirada de Acción:** +1 a tu puntuación de Tirada de Acción.
- **Especialización Técnica:** Tienes una bonificación permanente de -1 paso en las tiradas de especialidad de una de las siguientes habilidades amplias: Ciencias de la Computación, Ciencias de la Vida, Ciencias Médicas, Ciencias Físicas o Ciencia Técnica. La habilidad elegida influirá en gran medida en tu reputación futura.

## Habilidades Amplias Favorecidas (Coste: Precio Estándar - 1)
- **Acrobacias**
- **Ciencias de la Computación**
- **Demoliciones**
- **Ciencias de la Vida**
- **Ciencias Médicas**
- **Navegación**
- **Ciencias Físicas**
- **Seguridad**
- **Operación de Sistemas**
- **Tácticas**
- **Ciencia Técnica**
- **Operación de Vehículos**

## Especialidades Favorecidas (Coste: Precio Estándar - 1)
- **Acrobacias:** Entrenamiento en gravedad cero
- **Ciencias de la Computación:** (Todas)
- **Demoliciones:** Desactivar, Colocar explosivos
- **Ciencias de la Vida:** (Todas)
- **Ciencias Médicas:** (Todas)
- **Navegación:** Astrogación—espacio profundo, Astrogación—sistema
- **Ciencias Físicas:** (Todas)
- **Seguridad:** Dispositivos de seguridad
- **Operación de Sistemas:** (Todas)
- **Tácticas:** Tácticas espaciales
- **Ciencia Técnica:** (Todas)
- **Operación de Vehículos:** (Todas)

## Perks Favorecidos (Coste: Precio Estándar - 1)
- Enfoque en Especialidad (Specialty Skill Focus) (Cualquier habilidad de especialidad favorecida por esta profesión)
- Concentración (Concentration)
- Observador (Observant)
- Memoria Fotográfica (Photo Memory)
