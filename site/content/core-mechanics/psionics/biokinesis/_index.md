+++
title = "Biokinesis"
description = "Manipulate the user's or another living being's biology and physiology."
weight = 1
attribute = "CON"
category = "Psionics"
untrained = true
+++

*This skill can't be used untrained.*

This broad skill allows a character to manipulate his or her own biology and physiology, or the biology and physiology of another living being.

## Specialty Skills

- **[Bio-Armor](bio-armor)**: Solidify the outer layer of skin to create a natural, protective layer.
- **[Bioweapon](bioweapon)**: Mutate hands into natural weapons like claws or stingers.
- **[Clamber](clamber)**: Generate tiny suckers on hands/feet to climb or run on vertical and inverted surfaces.
- **[Control Metabolism](control-metabolism)**: Control bodily functions like heart rate and breathing for various physical benefits.
- **[Heal](heal)**: Use bio-energy to repair cell damage and speed up natural recovery.
- **[Intangibility](intangibility)**: Vibrate molecules to pass through solid objects or become immune to physical harm.
- **[Morph](morph)**: Alter the physical appearance of the body, including size and features.
- **[Rejuvenate](rejuvenate)**: Reverse the aging process or restore youth to cells.
- **[Shatter](shatter)**: Direct bio-energy into solid objects or armor to make them break.
- **[Transfer Damage](transfer-damage)**: Transfer wounds or fatigue from one living being to another.
